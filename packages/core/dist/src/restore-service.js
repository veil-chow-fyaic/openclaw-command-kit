...
export class RestoreService {
    ...
    history;
    sessionsDir;
    constructor(gateway, history, sessionsDir = process.env.OPENCLAW_SESSIONS_DIR || path.join(homedir(), '.openclaw/agents/main/sessions')) {
    ...
    async restoreSession(actor, route, displayIndex) {
        ...
        const item = defaultItems.find((i) => i.displayIndex === displayIndex) ??
    ...
    _atomicWriteJsonWithBackup(storePath, data) {
    ...
    _cleanupOldBackups(storePath, keepCount) {
    ...
    async _readBackConfirmed(sessionKey, expectedSessionId) {
        try {
