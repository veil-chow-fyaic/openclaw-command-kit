export function cleanTranscriptText(text, role) {
    let value = text.trim();
    if (role === 'user') {
...
export function isGoodTitleSeed(text) {
    return isMeaningfulConversationText(text, { forTitle: true });
}
export function isGoodPreviewText(text) {
...
export function truncateForTitle(text, maxLen) {
    const cleaned = cleanDisplayText(text);
    if (cleaned.length <= maxLen)
        return cleaned;
    return cleaned.slice(0, maxLen) + '…';
}
function stripOpenClawMetadataEnvelope(text) {
    if (!text.includes('```'))
        return text;
...
function isMeaningfulConversationText(text, options) {
    const value = cleanDisplayText(text);
    if (value.length < 2)
        return false;
    if (options.forTitle && value.startsWith('/'))
        return false;
    if (/^\/(sessions|resume|whereami)(?:\s|$)/i.test(value))
        return false;
    if (value === '[图片]' || value === '[排队消息]')
        return false;
    if (value === 'NO_REPLY')
        return false;
    if (isOpenClawMetadata(value))
        return false;
    if (isPackageManagerNoise(value))
        return false;
    if (isGatewayOrRuntimeNoise(value))
        return false;
    if (isCommandReplyText(value))
        return false;
    return true;
}
function isOpenClawMetadata(value) {
...
function isPackageManagerNoise(value) {
...
function isGatewayOrRuntimeNoise(value) {
    if (/^(acp\s+)?gateway\s+progress\s+fact\b/i.test(value))
        return true;
    ...
    if (/^tool call error:/i.test(value))
        return true;
    if (/^command timeout\b/i.test(value))
        return true;
    if (/^http request timed out after\b/i.test(value))
        return true;
    if (/^__tabby_mcp_exec_ok__$/i.test(value))
        return true;
    return false;
}
function isCommandReplyText(value) {
    if (/^(最近)?可恢复的历史对话[（(]/.test(value))
        return true;
    if (/^当前聊天还没有可恢复的历史对话/.test(value))
        return true;
    if (/^发送 \/resume\b/.test(value))
        return true;
    if (/^历史对话命令$/.test(value))
        return true;
    if (/^\/sessions 只用于查看和搜索/.test(value))
        return true;
    if (/^用法：\/resume/.test(value))
        return true;
    if (/^已切换到历史对话/.test(value))
        return true;
    if (/^当前会话信息/.test(value))
        return true;
    if (/^无法确认当前用户身份/.test(value))
        return true;
    if (/^无法确认当前聊天范围/.test(value))
        return true;
    if (/^没有找到匹配 ".+" 的历史对话/.test(value))
        return true;
    return false;
}
//# sourceMappingURL=session-text.js.map